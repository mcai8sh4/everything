...
#include <stdio.h> // Whoop, very few lib dependancies!! (As planned)
#define ROW 5	// pixels in image
#define COL 5
#define LIMIT 255 // max colour component value
#define N_LENGTH 21 //limited to 255 max => max picture ~60px

// structure to hold eack pixels RGB value
typedef struct {
	unsigned char r,g,b;
} pix;

void next_name(char* str); // Select the next sequential filename
void output(pix ar[][COL]); // Display the output to STDOUT - mainly testing
...
void output_img(pix ar[][COL], char* fname)
{
	...
	(void) fprintf(fp, "P6\n%d %d\n%d\n", ROW, COL, LIMIT); // ppm image header
...
int inc_colour(pix *p)
{
	...
	return full; // if full=1 then current pixel is full!
}
